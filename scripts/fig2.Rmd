...
```{r}
singlem_bray <- vegdist(singlem_rplP, method = 'bray')
...
singlem_phylo <- phyloseq(sample_data(metadata %>% 
                          ...
                          otu_table(singlem_rplP, taxa_are_rows = FALSE))
...
```{r}
fticr_bray <- decostand(fticr_pos, method = 'total') %>% 
  vegdist(, method = 'bray')

fticr_permanova <- adonis2(fticr_bray ~ month,
                           data = metadata %>% 
                             filter(asv_sample %in% rownames(fticr_pos)))
fticr_permanova

nmds_fticr <- metaMDS(fticr_bray,
                      distance = 'bray',
                      k = 2,
                      try = 500, 
                      maxit = 999,
...
fig2 <- singlem_tax_plot + fticr_elcomp_pos_plot + new_alpha_plot +
